...
| 功能 | 说明 |
|---|---|
| 导入 | 上传 .xlsx/.xls/.csv 文件；**标准模板**：在导入页「下载导入模板」获取 `.xlsx`（首表「资产导入」+「填写说明」），按列名填写以保持与系统一致。自动识别列名（电脑编号必填；序列号缺失将写入「暂无」，品牌/型号/模板名称/配置等可选），批量导入资产并自动创建缺失部门。**未登记型号**：预检后可选择创建模板或仅导入资产。接口字段：`dryRun`、`createMissingTemplates`、`allowMissingTemplates`（multipart） |
...
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/import` | 导入 Excel（`file` + 可选 `dryRun` / `createMissingTemplates` / `allowMissingTemplates`） |
| GET | `/template` | 下载标准导入模板 `.xlsx`（任意登录用户；资产导入表 + 填写说明） |
...
3. **开发**：在 `backend` 目录使用 `pnpm run dev`（`tsx watch`）可立即加载最新源码；**若使用 `pnpm start`（运行 `dist/`）**，每次改过后端代码后须先执行 **`pnpm run build`**，否则会仍是旧路由（例如出现 `Cannot GET /api/excel/template`）。
4. **前端 `pnpm dev`（5173）**：`getApiBaseURL()` 在开发模式下为空，接口走 **`http://localhost:5173/api/...`**，由 Vite **代理**到后端（默认 `http://127.0.0.1:3000`）。浏览器里不必再写 `:3000`；后端仍须在后台运行。若后端不在 3000，可在 `frontend/.env.development` 设置 `VITE_API_PROXY_TARGET=http://127.0.0.1:你的端口`。若必须直连后端域名，可设 `VITE_API_BASE_URL`。
5. 后端启动后同时托管前端静态文件，单端口访问（默认 3000）
6. 生产部署可使用 pm2 进程管理器保持后台运行
