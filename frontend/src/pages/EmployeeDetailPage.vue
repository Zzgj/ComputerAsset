<template>
  <div class="ca-page ca-animate">
      ...
      <div class="header-row">
        <div class="info-col">
          ...
          <div class="tag-row">
            ...
            <el-tag v-if="employee.campus?.name" type="primary" effect="plain">{{ employee.campus.name }}</el-tag>
            <el-tag v-if="employee.department?.name" effect="plain">{{ employee.department.name }}</el-tag>
          </div>
          <div v-if="employee.joinedAt || employee.resignedAt" class="date-row">
            ...
            <span v-if="employee.resignedAt" class="resigned-date">离职：{{ formatDate(employee.resignedAt) }}</span>
          </div>
        ...
        <div class="action-col">
          ...
          <el-button v-if="canTransfer" plain type="primary" @click="openTransferCampus">修改园区</el-button>
          <el-button
    ...
    <el-card v-if="employee" shadow="never" class="section-card">
      ...
      <el-table :data="assets" size="small" v-loading="loadingAssets" empty-text="该员工名下暂无 IT 资产">
        <el-table-column prop="assetCode" label="资产编号" min-width="140">
          <template #default="{ row }">
            <router-link :to="`/assets/${row.id}`" class="row-link">{{ row.assetCode }}</router-link>
          </template>
        </el-table-column>
    ...
    <el-card v-if="employee" shadow="never" class="section-card">
      <div class="section-header">
        ...
        <el-button v-if="canWrite && employee.status === 'active'" type="primary" size="small" @click="openResourceAdd">
          + 新增资源
        </el-button>
      </div>
      <el-table :data="resources" size="small" empty-text="暂无外部资源">
        ...
        <el-table-column label="操作" width="220" fixed="right">
          <template #default="{ row }">
            <el-button v-if="canWrite" size="small" text type="primary" @click="openResourceEdit(row)">
              编辑
            </el-button>
            <el-button
    ...
    <!-- 新增/编辑资源 -->
    <el-dialog v-model="resourceDialogVisible" :title="resourceForm.id ? '编辑外部资源' : '新增外部资源'" width="480px">
      <el-form :model="resourceForm" label-width="90px">
        ...
        <el-form-item label="标识/号码" required>
          <el-input v-model="resourceForm.identifier" maxlength="255" placeholder="如手机号、邮箱地址、域控账号等" />
        </el-form-item>
      ...
      <div class="resign-body">
        <div v-if="resignAssets.length" class="resign-section">
          ...
          <div class="resign-hint">
            请优先通过资产详情页办理归还/调拨。如确需保留资产记录强制离职，请勾选下方「强制完成」。
          </div>
          <el-table :data="resignAssets" size="small">
            <el-table-column prop="assetCode" label="资产编号" min-width="140">
              <template #default="{ row }">
                <router-link :to="`/assets/${row.id}`" class="row-link" target="_blank">{{ row.assetCode }}</router-link>
              </template>
            </el-table-column>
            <el-table-column label="状态" width="120">
              <template #default="{ row }">
        ...
        <div class="resign-section">
          ...
          <el-input v-model="resignForm.remark" type="textarea" :rows="2" placeholder="可选，离职说明" />
        </div>
      </div>
      <template #footer>
...
async function reload() {
  ...
  try {
    const data = await apiRequest<{ employee: any; assets: any[] }>(`/api/employees/${employeeId.value}`)
